...
$("moreBtn").onclick = function(){
  ...
  $("moreBtn").textContent = e.classList.contains("on") ? L.less : L.more;
};
...
function addItem(){
  ...
  if(!v){ err(L.errEmpty); $("what").focus(); return; }
  items.push({ value:v, qty: parseInt($("qty").value,10) || 1, box: $("box").value.trim(), note: $("note").value.trim() });
  $("what").value = ""; setQty(1); $("note").value = "";
  render(); $("what").focus();
}
$("add").onclick = addItem;
...
function render(){
  ...
  items.forEach(function(it, i){
    ...
    row.className = "item";
    row.innerHTML = '<div class="main"><div class="v"></div>' + (meta ? '<div class="m"></div>' : '') + '</div>' +
      '<div class="q">&times;' + it.qty + '</div><button type="button" class="x">&times;</button>';
  ...
  var tot = items.reduce(function(s,x){ return s + x.qty; }, 0);
  $("tItems").textContent = tot;
  $("tLines").textContent = items.length + " " + (items.length === 1 ? L.line : L.lines);
  $("cnt").textContent = items.length ? (items.length + " " + L.lines + " · " + tot + " " + L.items) : L.none;
...
$("photo").addEventListener("change", function(e){
  var files = [].slice.call(e.target.files, 0, 4), chain = Promise.resolve();
...
function openSheet(){
  ...
  $("sMeta").textContent = CFG.name + " · " + dt + (bx ? (" · " + bx + " " + L.boxWord) : "");
...
$("send").onclick = function(){
  ...
  var payload = {
    supplier: CFG.name, type: CFG.type, group: CFG.group,
    date: $("date").value,
    ...
    .catch(function(){
      fetch(WEBHOOK, { method:"POST", mode:"no-cors", headers:{"Content-Type":"text/plain"}, body: JSON.stringify(payload) })
        ...
        .catch(function(){
          ...
          $("sheet").classList.remove("on"); err(L.errSend);
        });
    });
  function ok(){
